...
$nim = $_SESSION['nim'];
$sql_active = "SELECT COUNT(*) as count FROM lab_bookings WHERE nim='$nim' AND status='Disetujui'";
$result_active = $conn->query($sql_active);
$active_count = $result_active->fetch_assoc()['count'];

$sql_completed = "SELECT COUNT(*) as count FROM lab_bookings WHERE nim='$nim' AND status='Selesai'";
$result_completed = $conn->query($sql_completed);
$completed_count = $result_completed->fetch_assoc()['count'];

$sql_pending = "SELECT COUNT(*) as count FROM lab_bookings WHERE nim='$nim' AND (status='Menunggu Persetujuan Laboran' OR status='Menunggu Persetujuan Kalab')";
$result_pending = $conn->query($sql_pending);
$pending_count = $result_pending->fetch_assoc()['count'];

$sql_rejected = "SELECT COUNT(*) as count FROM lab_bookings WHERE nim='$nim' AND status='Ditolak'";
$result_rejected = $conn->query($sql_rejected);
$rejected_count = $result_rejected->fetch_assoc()['count'];

// Mendapatkan peminjaman terbaru
$sql_recent = "SELECT id, jenis_ruangan, hari_tanggal_mulai, status, keterangan FROM lab_bookings WHERE nim='$nim' ORDER BY hari_tanggal_mulai DESC LIMIT 10";
                                    ...
                                    <table class="table table-bordered" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = $result_recent->fetch_assoc()) :
                                                ...
                                                $tanggal = $hari_tanggal_mulai->format('d-m-Y');
                                            ?>
                                                <tr>
